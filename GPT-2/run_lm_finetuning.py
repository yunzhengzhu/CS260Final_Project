...
class TextDataset(Dataset):
	def __init__(self, tokenizer: PreTrainedTokenizer, args, file_path: str, block_size=512):
		...
		if os.path.exists(cached_features_file) and not args.overwrite_cache:
			...
			self.examples = []
			with open(file_path+"-x.csv", encoding="utf-8") as f:
				text = f.read()
...
class LineByLineTextDataset(Dataset):
	def __init__(self, tokenizer: PreTrainedTokenizer, args, file_path: str, block_size=512):
		...
		self.examples = []
		for i in lines:
			self.examples.append(tokenizer.tokenize(i,max_length = block_size)["input_ids"])
...
class ComsenTextDataset(Dataset):
	def __init__(self, tokenizer: PreTrainedTokenizer, args, file_path: str, block_size=512):
		...
		for i in lines:
			#pdb.set_trace()
			self.examples.append(tokenizer.encode_plus(i, max_length=block_size)["input_ids"])
	def __len__(self):
		return len(self.examples)
